<div class="mt-2">
                ...
                @foreach ($parties as $party)
                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200">
                        ...
                        <td class="px-6 py-4">
                            <ul>
                                <li>
                                    <a wire:click='showForm({{ $party->id }})'
                                ...
                                <li>
                                    <a wire:click='showDelete({{ $party->id }})'
    ...
    <flux:modal name="party-form" class="md:w-120">
        <div class="space-y-6">
            <div>
                @if ($activity == 'create')
                    <flux:heading size="lg">{{ __('Create party') }}</flux:heading>
                @else
                    <flux:heading size="lg">{{ __('Edit party') }}</flux:heading>
                @endif
            </div>
            ...
            <flux:input wire:model='form.acronym' label="{{ __('Acronym') }}"
                placeholder="{{ __('Example') }}: JD" />
    ...
    <flux:modal name="party-delete" class="md:w-120">
        <div class="space-y-6">
            <div>
                <flux:heading size="lg">{{ __('Delete party') }}</flux:heading>
