<div class="mt-2">
                            ...
                            <ul class="max-w-md space-y-1 text-gray-500 list-disc list-inside dark:text-gray-400">
                                @foreach ($user->stations as $station)
                                    <li class="uppercase text-white">
                                        {{ $station->enclosure->name }} - {{ $station->name }} <a
                                            wire:click='showDelete({{ $user->id }} , {{ $station->id }})'
    ...
    <flux:modal name="assignment-form" class="md:w-120">
        <div class="space-y-6">
            <div>
                <flux:heading size="lg">{{ __('Station assignment') }}</flux:heading>
            </div>

            <flux:label>{{ __('Enclosure') }}</flux:label>

            <flux:select class="mt-2" wire:model.live="enclosure">
            ...
            @if ($stations->count() > 0)
                ...
                <flux:select class="mt-2" wire:model="station">
                    <flux:select.option value="">{{ __('Select a station') }}</flux:select.option>
