<div class="mt-2">
            ...
            <tbody>
                @foreach ($stations as $station)
                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200">
                        ...
                        <td class="px-6 py-4">
                            {{ $station->enclosure->name }}
                        </td>
                        <td class="px-6 py-4">
                            <ul>
                                <li>
                                    <a wire:click='showForm({{ $station->id }})'
                                ...
                                <li>
                                    <a wire:click='showDelete({{ $station->id }})'
    ...
    <flux:modal name="station-form" class="md:w-120">
        <div class="space-y-6">
            <div>
                @if ($activity == 'create')
                    <flux:heading size="lg">{{ __('Create station') }}</flux:heading>
                @else
                    <flux:heading size="lg">{{ __('Edit station') }}</flux:heading>
                @endif
            </div>

            <flux:label>{{ __('Enclosure') }}</flux:label>

            <flux:select class="mt-2" wire:model="form.enclosure">
            ...
            <flux:error name="form.enclosure" />
    ...
    <flux:modal name="station-delete" class="md:w-120">
        <div class="space-y-6">
            <div>
                <flux:heading size="lg">{{ __('Delete station') }}</flux:heading>
