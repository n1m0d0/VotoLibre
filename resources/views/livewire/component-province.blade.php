<div class="mt-2">
                ...
                @foreach ($provinces as $province)
                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200">
                        ...
                        <td class="px-6 py-4">
                            <ul>
                                <li>
                                    <a wire:click='showForm({{ $province->id }})'
                                ...
                                <li>
                                    <a wire:click='showDelete({{ $province->id }})'
    ...
    <flux:modal name="province-form" class="md:w-120">
        <div class="space-y-6">
            <div>
                @if ($activity == 'create')
                    <flux:heading size="lg">{{ __('Create province') }}</flux:heading>
                @else
                    <flux:heading size="lg">{{ __('Edit province') }}</flux:heading>
                @endif
            </div>

            <flux:label>{{ __('Department') }}</flux:label>

            <flux:select class="mt-2" wire:model="form.department">
                <flux:select.option value="">{{ __('Select a department') }}</flux:select.option>
    ...
    <flux:modal name="province-delete" class="md:w-120">
        <div class="space-y-6">
            <div>
                <flux:heading size="lg">{{ __('Delete province') }}</flux:heading>
