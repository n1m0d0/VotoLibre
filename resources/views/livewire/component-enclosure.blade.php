<div class="mt-2">
                ...
                @foreach ($enclosures as $enclosure)
                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200">
                        ...
                        <td class="px-6 py-4">
                            <ul>
                                <li>
                                    <a wire:click='showForm({{ $enclosure->id }})'
                                ...
                                <li>
                                    <a wire:click='showDelete({{ $enclosure->id }})'
    ...
    <flux:modal name="enclosure-form" class="md:w-120">
        <div class="space-y-6">
            <div>
                @if ($activity == 'create')
                    <flux:heading size="lg">{{ __('Create enclosure') }}</flux:heading>
                @else
                    <flux:heading size="lg">{{ __('Edit enclosure') }}</flux:heading>
                @endif
            </div>

            <flux:label>{{ __('Zone') }}</flux:label>

            <flux:select class="mt-2" wire:model="form.zone">
                <flux:select.option value="">{{ __('Select a zone') }}</flux:select.option>
    ...
    <flux:modal name="enclosure-delete" class="md:w-120">
        <div class="space-y-6">
            <div>
                <flux:heading size="lg">{{ __('Delete enclosure') }}</flux:heading>
